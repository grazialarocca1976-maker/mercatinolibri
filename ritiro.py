...
def genera_pdf_ricevuta(dati_cliente, libri_ritirati):
    buffer = BytesIO()
...
def mostra_pagina():
    ...
    with tab_ritiro:
                    ...
                    with st.spinner("Salvataggio..."):
                        ...
                        for item in st.session_state["carrello_ritiro"]:
                            ...
                            if res_ins.status_code < 400:
                                ...
                                # include anche l'id numerico del cliente per creare un barcode univoco: <client_id>-<codice_personale>-<id_libro>
                                libri_per_ricevuta.append({
                                    "etichetta": f"{id_generato} - {dati_cliente['codice_personale']}",
                                    "isbn": item['isbn'], 
                                    "titolo": item['titolo'], 
                                    "prezzo": item['prezzo'],
                                    "codice_personale": dati_cliente['codice_personale'],
                                    "id_venditore": dati_cliente.get('id'),
                                    "barcode": f"{dati_cliente.get('id')}-{dati_cliente['codice_personale']}-{id_generato}"
                                })
        ...
        if st.session_state["libri_appena_salvati"]:
            ...
            with col_pdf_f:
                pdf_f_data = genera_pdf_ricevuta(dati_cliente, st.session_state["libri_appena_salvati"])
            ...
            with col_a4:
                layout_scelto = st.selectbox(
                    ...
                    ["Standard (3x8)", "A5 compatibile (2x4)"],
                    index=0,
                    help="Se la tua stampante non trova le etichette, prova il layout compatibile A5 o modifica le misure.",
                )
                if st.button("📄 STAMPA SU FOGLIO A4", use_container_width=True):
                    layout = None if layout_scelto == "Standard (3x8)" else "a5"
                    esito = genera_griglia_a4(st.session_state["libri_appena_salvati"], file_output="etichette_a4_marconi.pdf", stampa=True, layout=layout)
                    if esito:
                        st.success("PDF etichette A4 creato. Se la stampante non trova il layout, prova il preset A5 compatibile.")
                    else:
                        st.warning("Non è stato possibile creare il PDF etichette A4. Controlla i dati o il percorso di output.")
                ...
                if st.button("⬇️ SCARICA PDF ETICHETTE (PER STAMPA MANUALE)", use_container_width=True):
                    ...
                    try:
                        ...
                        pdf_bytes = genera_bytes(st.session_state["libri_appena_salvati"], layout=layout)
    ...
    with tab_inventario:
        ...
        res_cat = requests.get(f"{URL_REST}/catalogo_libri?select=isbn,titolo", headers=HEADERS)
        if res_copie.status_code == 200 and res_cat.status_code == 200:
            ...
            if len(copie_dati) > 0:
                ...
                if res_cl_all.status_code == 200 and res_cl_all.json():
                    ...
                    df_totale['Codice Etichetta'] = df_totale['id_libro'].astype(str) + " - " + df_totale['codice_personale'].astype(str)
                    df_totale = df_totale[['Codice Etichetta', 'isbn', 'titolo', 'prezzo_inserito_mano', 'stato']]
                    df_totale.columns = ['Codice Copertina', 'ISBN', 'Titolo', 'Prezzo Copertina (€)', 'Stato']
